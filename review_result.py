...
class ReviewInput:
    ...
    result_list="result.txt"
    iou_thres=0.5
    conf_thresh=0.01
    gt_path="Mine_test.txt"
    image_dir="VOCMine/Annotations"
    type='on'

class Reviewer:

    parser = argparse.ArgumentParser(description='Object Detection Calculation')
    parser.add_argument('-r', '--result_list', default=None, type=str, help='path of the result list file')
    parser.add_argument('-th', '--iou_thres', default=0.5, type=float, help='threshold for IoU, Defatut:0.5')
    parser.add_argument('-c', '--conf_thresh', default=0.01, type=float, help='threshold for confidence score, Default: 0.01')
    parser.add_argument('-g', '--gt_path', default=None, type=str, help='path of the anotation list file')
    parser.add_argument('-i', '--image_dir', default=None, type=str, help='path of the image annotation folder')
    parser.add_argument('-t', '--type', default='jueyuanzi', type=str, help='type of the class to calculate and show')
    ...
    def review(self,opt):
        ...
        for idx, path in enumerate(gt_file):
            ...
            for ix, obj in enumerate(objs):
                ...
                cls_idx = str(self.get_data_from_tag(obj, "name")).lower().strip()
                ...
                for result in results:
                    ...
                    if score >= thres:
                        ...
                        for gt_box, gt_cls in zip(boxes, gt_classes):
                            iou = self.bb_intersection_over_union(pred_box, gt_box.tolist())
